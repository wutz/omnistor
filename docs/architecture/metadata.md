# 元数据设计（Bucket 分片 × DASE 持久化）

OmniStor 没有独立的"元数据服务器"层：元数据被切分为大量 **Bucket**，全量分布到集群**所有 TLC NVMe SSD** 上；Bucket 的持久化状态放在 DASE 共享 NVMe 池中，使 Bucket 的接管（failover）无需任何数据迁移。
...
| 线性扩展 | 对象数、元数据 IOPS 随节点/SSD 数量近线性增长，支撑 10 万亿（10¹³）对象 |
...
与 shared-nothing 系统（Bucket 状态落在本节点 SSD，failover 需依赖纠删重建）不同，OmniStor 把 Bucket 状态放在 **DASE 共享 NVMe 池**：
...
- 元数据块本身的冗余由数据层统一的纠删/副本机制保证（跨 SNode 故障域），Bucket 层不再需要 RAFT 复制——单写者（single-writer per bucket）+ 共享持久化 取代了多副本状态机。

## 元数据容量：跟随使用量增长

- 元数据与数据**共用同一个 TLC NVMe 池**，没有为元数据划出固定分区或固定百分比。
- 池按固定大小的**分配单元（extent，如 64 MB）**管理，Bucket 需要空间时向池申请，删除/合并后归还——元数据占用量精确等于实际使用量。
- 空文件系统的元数据开销近乎为零；10 万亿小对象场景下元数据可自然增长到 ~10 PB 量级，不需要提前容量规划。
...
- **加 SSD/SNode**：新 SSD 纳入所属池后，新分配的元数据 extent 自动落在新盘上；后台可选做 extent 级重平衡以摊平磨损。
- **多 TLC 池**：集群存在多个 TLC 池（不同代际/规格）时，元数据 extent 按各池容量水位与 IOPS 余量加权分配，仍覆盖全部 TLC SSD；单池故障只影响其上的元数据分片（见 [pools.md](pools.md)）。
- **10 万亿对象推演**：单 Bucket 承载 ~1 亿对象（B-tree 深度 3–4），10 万亿对象约需 10 万个 Bucket；冷 Bucket 休眠换出后，按每节点常驻 64–128 个活跃 Bucket 计，千级 CNode 即可承载全量活跃元数据处理，见 [scaling.md](../scaling.md)。

...
┌─────────────────────────▼────────────────────────────────┐
│  SNode 共享存储层（DASE，通用 x86 存储服务器）              │
│  按硬件规格分池: pool tlc-gen1 · tlc-gen2 · qlc-a · hdd-a  │
│  主层: TLC NVMe 池（元数据+数据，动态分配，无固定边界）      │
│  分层: QLC NVMe / HDD 池（可选）                           │
└─────────────────────────┬────────────────────────────────┘
                          │ 可选归档外溢
                ┌─────────▼─────────┐
                │ 外部对象存储 (S3)  │
...
| 支柱 | 内容 |
| --- | --- |
| **DASE 共享存储** | 计算/存储解耦；全部 SSD 经 NVMe-oF 全局共享；无状态 CNode；通用 x86 存储服务器 |
...
| 10 万亿对象 / 10 EiB | Bucket 横向扩展 + SNode 堆叠，见 [scaling.md](../scaling.md) |
| 异构硬件共集群 | 按硬件规格分池，池间负载/容量均衡，见 [pools.md](pools.md) |
...
| 多租户 | 租户级命名空间/VIP/认证/密钥隔离，两级 QoS 与 Quota，见 [features/multitenancy.md](../features/multitenancy.md) |
| 数据保护 | 分布式 D+P 纠删、写新位置、全员并行重建（池内），见 [data-protection.md](data-protection.md) |
| 快照/克隆 | 元数据 COW 快照瞬时创建、可写克隆、snap-to-object，见 [features/snapshots.md](../features/snapshots.md) |

## 相关文档

- [DASE 详解](dase.md)
- [元数据设计（Bucket 分片）](metadata.md)
- [数据保护（纠删与并行重建）](data-protection.md)
- [硬件方案（通用 x86）](hardware.md)
- [硬件分池与池间均衡](pools.md)

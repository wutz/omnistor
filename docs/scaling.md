...
- SNode 按硬件规格分池（Storage Pool），池内同构、池间异构——同一集群可混用不同规格/代际的硬件，分代扩容即新增一个池（见 [architecture/pools.md](architecture/pools.md)）。
- 新 SNode 加入所属池后即纳入共享层，新写入的纠删条带自动分散到新 SSD；放置引擎在同 media class 的多个池间按容量水位与负载加权均衡，存量按需后台再平衡。
- 配置外部 S3 层后冷数据外溢，本地容量压力有上界——10 EiB 目标中冷数据占比越高，本地 SNode 需求越小。
- 纠删组在**池内**跨 SNode 构建，单 SNode 故障不丢数据；重建由所有 CNode 并行分担，重建流量限制在池内。
- 千级 SNode 的超大池再切分 **zone**：纠删组在 zone 内构建，放置与重建流量进一步收敛，避免泛洪。
